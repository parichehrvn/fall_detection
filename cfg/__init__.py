...
@click.option('--output', type=Path, required=True, help='Path to save output directory.')
@click.option('--num', type=int, required=False, default=15, help='Number of key frames to extract from each video.')
@click.option('--model', type=Path, required=False, default=None, help='Path to Pose Estimation model. If set, a pose estimation model is used to generate keypoints for each frame.')
def extractData(input, output, num, model):
    """
    Extract key frames from videos in split dataset. If pose model provided, it also extracts keypoints for each frame.
    ...
    :param num: Number of key frames to extract from each video.
    :param keypoints: If True, a pose estimation model is used to generate keypoints for each frame.
    :param model: Path to pose estimation model.
    """
    ...
    for split in ['train', 'val', 'test']:
        ...
        for category in ['ADL', 'FALL']:
            ...
            for video_path in tqdm(video_dir.iterdir(), desc=f'Saving {split}/{category} frames'):
                ...
                extract_and_save_key_frames(video_path, save_path, num_keyframes=num)
...
@click.option('--seq_len', type=int, required=False, default=15, help='Number of frames in the sequence. It should be the same with --num in "extract" command.')
def fall_detection_inference(source, pose_model, tcn_model, save_dir, show_video, seq_len):
    main_inference(source, pose_model, tcn_model, save_dir, show_video, seq_len)
...
if __name__ == '__main__':
    main.add_command(tcn)
    main.add_command(fallDetection)
    main()
